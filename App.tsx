...
import { loadCourses } from './src/services/CourseLoader';

const App: React.FC = () => {
  const [progress, setProgress] = useState<UserProgress>(() => {
    const saved = localStorage.getItem('user_investment_progress');
    return saved ? JSON.parse(saved) : { completedLessons: [], currentLessonId: 'lesson-1' };
  });
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const loadedModules = await loadCourses();
        setModules(loadedModules);
        
        // 设置默认课程
        if (loadedModules.length > 0 && loadedModules[0].lessons.length > 0) {
          const id = progress.currentLessonId || loadedModules[0].lessons[0].id;
          const foundLesson = loadedModules
            .flatMap(mod => mod.lessons)
            .find(l => l.id === id);
          setActiveLesson(foundLesson || loadedModules[0].lessons[0]);
        }
      } catch (error) {
  ...
  const handleContinueLearning = () => {
    ...
    const currentLesson = allLessons.find(l => l.id === progress.currentLessonId);
    if (currentLesson) {
        ...
        {isLoading ? (
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              ...
              <p className="text-lg text-slate-600">正在加载课程...</p>
            </div>
          </div>
        ) : (
                      ...
                      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 w-64">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className="text-xs text-slate-400">
                            {modules.flatMap(m => m.lessons).findIndex(l => l.id === activeLesson.id) + 1} / {totalLessons}
                          </span>
                        </div>
